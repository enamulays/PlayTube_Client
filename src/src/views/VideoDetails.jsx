...
const VideoDetails = () => {
  ...
  const addVideoComment = (data) => {
    console.log(data);
    setComments((prev) => [data, ...prev]);
  };

  const hasLikedVideo = (video) => {
    return video.likedBy.some((likeUser) => likeUser._id === user?._id);
  };

  // Function to check if the user has disliked a video
  const hasDislikedVideo = (video) => {
    return video.disLikedBy.some((likeUser) => likeUser._id === user?._id);
  };
  ...
  const handleToggleLike = async () => {
    try {
      const data = await toggleVideoReaction(videoId, "like");
      console.log(data.data);
    } catch (error) {
      console.log(`Error log while toggle like`, error);
    }
  };

  const handleDislikeToggle = async () => {
    try {
      const data = await toggleVideoReaction(videoId, "dislike");
      console.log(data.data);
    } catch (error) {
      console.log(`Error log while toggle unlike`, error);
    }
  };
  ...
  const addLike = (data) => {
    ...
    const { like, totalLike, totalUnlike } = data;
    setLikesVideos((prev) => {
      const isDuplicate = prev.some((like) => like._id === like._id);
      if (!isDuplicate) {
        return [like, ...prev];
      }
      return prev;
  ...
  useEffect(() => {
    ...
    socket.on(SocketEventEnum.SOCKET_CONNECTED, onConnect);
    socket.on(SocketEventEnum.SOCKET_DISCONNECTED, onDisconnect);
    socket.on(SocketEventEnum.ADD_VIDEO_COMMENT, addVideoComment);
    socket.on(SocketEventEnum.ADDED_LIKE, addLike);
    socket.on(SocketEventEnum.ADDED_DISLIKE, addDislike);

    return () => {
      console.log("Cleaning up socket listeners");

      socket.off(SocketEventEnum.SOCKET_CONNECTED, onConnect);
      socket.off(SocketEventEnum.SOCKET_DISCONNECTED, onDisconnect);
      socket.off(SocketEventEnum.ADD_VIDEO_COMMENT, addVideoComment);
      socket.off(SocketEventEnum.ADDED_LIKE, addLike);
      socket.off(SocketEventEnum.ADDED_DISLIKE, addDislike);
    };
  }, [socket, videoId]);
    ...
    (async () => {
      if (videoId) {
        try {
          ...
          const videoReaction = await getLikeCountOnVideo(videoId);
          setVideo(data?.data);
          setLikeCount(videoReaction.data?.data?.totalLikes);
          setDislikeCount(videoReaction.data?.data?.totalDislikes);
          setLikesVideos(videoReaction.data?.data?.likesVideo);
        } catch (error) {
  ...
  const handleKeyPress = async (e) => {
    if (e.key === "Enter") {
      ...
      // Call your API to add the comment
      try {
        await addComment(videoId, { content: commentContent });
                  ...
                  >
                      ...
                      <div class="w-full md:w-1/2 lg:w-full xl:w-1/2">
                        <div class="flex items-center justify-between gap-x-4 md:justify-end lg:justify-between xl:justify-end">
                          <div class="flex overflow-hidden rounded-lg border">
                            <button
                              class="group/btn flex items-center gap-x-2 border-r border-gray-700 px-4 py-1.5  hover:bg-white/10"
                            ...
                            <button
                              ...
                              class="group/btn flex items-center gap-x-2 px-4 py-1.5 hover:bg-white/10"
                            >
                              <span
                                class={`inline-block w-5 ${
                                  isDislike && "text-[#ae7aff]"
                                }`}
                              >
                                <svg
                                  xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div class="mt-4 flex items-center justify-between">
                      <div class="flex items-center gap-x-4">
                        ...
                        <div class="block">
                          <p class="text-gray-200">{video?.owner?.fullName}</p>
